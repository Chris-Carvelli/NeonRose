...
{
    /// <summary>
    /// An action that makes an agent go through a list of locations sequentially. 
    /// When reaching the last location, loops back to the first location and starts over.
    /// </summary>
    public class MoveMultiLoopAction : SingletonMoveAction
    {
        [Header("General")]
        ...
        private void EvaluateAgentRotation()
        {
            if (RotateBeforeMove == true)
            {
                ...
                else
                {
                    ...
                    Quaternion rotationCurrent = transform.rotation;
                    transform.rotation = Quaternion.RotateTowards(rotationCurrent, difference * rotationCurrent, rotateSpeed * Time.deltaTime);
                }
            }
            else
            {
